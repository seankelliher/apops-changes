...
if ( ! function_exists( 'pops_submission_comment' ) ) :
/**
 * Template for comments and pingbacks.
 *
 * Used as a callback by wp_list_comments() for displaying the comments.
 */
function pops_submission_comment( $comment, $args, $depth ) {
                ...
                <header>
                    <div class="comment-meta">
                        <a class="date" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                            <time pubdate datetime="<?php comment_time( 'c' ); ?>"><?php printf( __( '%1$s', 'maspops' ), get_comment_date( 'm.d.y' ) ); ?></time>
                        </a>
                    <?php printf( __( '<span class="by">submitted by:</span> <span class="author">%s</span>', 'maspops' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
                    </div><!-- .comment-meta -->
                    <?php if ( $comment->comment_approved == '0' ) : ?>
                        <em><?php _e( 'Your submission is awaiting moderation.', 'maspops' ); ?></em>
                            <br />
                    <?php endif; ?>
        ...
        <li class="post pingback">
          <p>
            <?php _e( 'Pingback:', 'maspops' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __('(Edit)', 'maspops'), ' ' ); ?>
          </p>
        <?php
                 break; // break pingback and trackback
        endswitch;
}
...
<?php if ( post_password_required() ) : ?>
    <div class="nopassword">
        <?php _e( 'This post is password protected. Enter the password to view any comments.', 'maspops' ); ?>
    </div>
<?php else: ?>

<?php // You can start editing here -- including this comment! ?>
    
    <?php comment_form( pops_comment_form_params() ); ?>

    <?php if ( have_comments() ) : ?>
        
        <h3 class="user-submission-title">
            <?php
                printf( _n( '<strong>1</strong> User Submission', '<strong>%1$s</strong> User Submissions', get_comments_number(), 'maspops' ), number_format_i18n( get_comments_number() ) );
            ?>
        </h3>
        ...
        <ol class="comment-list">
                <?php wp_list_comments( array( 'callback' => 'pops_submission_comment' ) ); ?>
        </ol>
...
<?php endif; // end if password protected ?>
    </div><!-- #comments -->
